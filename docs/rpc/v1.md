...
```javascript
const res = await api.call( "/v1/nginx/clear-cache" );
```
...
curl \
    -H "Authorization: Bearer <YOUR-API-TOKEN>" \
    "http://localhost/api/v1/nginx/clear-cache"
```
...
```javascript
const res = await api.call( "/v1/storage/download", path );
```
...
curl \
    -H "Authorization: Bearer <YOUR-API-TOKEN>" \
    "http://localhost/api/v1/storage/download"
```
...
```javascript
const res = await api.call( "/v1/storage/upload", path, file, options? );
```
...
curl \
    -H "Authorization: Bearer <YOUR-API-TOKEN>" \
    "http://localhost/api/v1/storage/upload"
```
...
- `file` {Message} Maximim message content length: `NaN` bytes.
